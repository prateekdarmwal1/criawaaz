...
<div class="mws-panel-header" style="margin-top:10px;">
    <span style="width:300px;float:left" class="mws-i-24 mws-report-icon mws-ic-16 ic-calculator">Match & Session Comments</span>
</div>
<div class="mws-panel-body" style="display: inline-flex;width:99.5%;">
  <div style="margin-left: 20px; padding-top:10px;padding-bottom: 10px; width: 50%">
      <label style="margin-bottom:10px;font-size:16px;">Please add match comments (if any): </label>
      <textarea style="width:90%;height:100px;" id="match_detail" disabled><?= trim($score['Score']['match_detail'])?></textarea><br>
      <button class="mws-button red" onclick="submitMatchComments(this)">Edit</button>
  </div>
  <div style="padding-top:10px;padding-bottom: 10px; width: 50%">
    <label style="margin-bottom:10px;font-size:16px;">Please add session comments (if any): </label>
      <textarea style="width:90%;height:100px;" id="session_comment" disabled><?= trim($score['Score']['session_detail'])?></textarea><br>
      <button class="mws-button red" onclick="submitSessionComments(this)">Edit</button>
  </div>
</div>
...
<script>
        function submitMatchComments(e){
        ...
        function submitSessionComments(e){
          if($('#session_comment').is(':disabled')){
            $('#session_comment').attr('disabled',false);
            $('#session_comment').focus();
            $(e).text("Submit");
            $(e).removeClass("red");
            $(e).addClass("green");
          }
          else{
            $('#session_comment').attr('disabled',true);
            $(e).text("Edit");
            $(e).removeClass("green");
            $(e).addClass("red");
            callAjax(UpdateInnings);
